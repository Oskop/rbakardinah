...
```mermaid
flowchart TD
    A[Admin: Buka Periode] --> B[Operator: Isi Latar Belakang & Rincian Belanja & Upload PDF Rincian Usulan]
    B --> C[Operator: Submit Usulan ke Supervisor]
    C --> D{Supervisor Acc / Tidak?}
    D -- Ya --> E[Supervisor: Review & Validasi Rincian Belanja]
    D -- Tidak --> F[Operator: Upload PDF Revisi/Penyesuaian]
    F --> C[Operator: Submit Usulan ke Supervisor]
    E --> G[Supervisor: Validasi Submissions Unit]
    G --> H[Admin: Lock Status Global RBA Header / Set Pagu per Nomor Rekening]
    H --> I[Operator: Upload Dokumen KAK, RAK, RTP Per Operator]
    I --> J[Supervisor: Review Dokumen Per Operator Bawahan]
```
