...
<head>
    ...
    <title>Laporan Rincian Belanja Usulan RBA Administrator - RSUD Kardinah ({{ $header->year ?? '' }})</title>
    <style>
        /* ==========================================================================
           SIPAKAR ADMINISTRATOR RBA REPORT PRINT TEMPLATE
           ========================================================================== */
...
<body>
    ...
    <div class="no-print-bar">
        <div>
            <h3>🖨️ Pratinjau Cetak Administrator - Rincian Belanja RBA</h3>
        </div>
        <div style="display: flex; gap: 10px;">
            <a href="{{ route('admin.headers.show', $header->id) }}" class="btn-action btn-back">
                ← Kembali ke Detail RBA Admin
            </a>
    ...
    <div class="page-container">
        ...
        <div class="report-title">
            ...
            <p>TAHUN ANGGARAN {{ $header->year ?? date('Y') }}</p>
        </div>
        ...
        <table class="meta-table">
            <tr>
                ...
                <td class="meta-value">: <strong>{{ $header->year ?? '-' }}</strong></td>
                <td class="meta-label">PERIODE RBA</td>
                <td class="meta-value">: {{ $header->period->name ?? '-' }}</td>
            </tr>
            <tr>
                <td class="meta-label">ADMINISTRATOR PENGELOLA</td>
                <td class="meta-value">: {{ Auth::user()->name ?? '-' }} ({{ Auth::user()->email ?? '' }})</td>
                <td class="meta-label">TANGGAL CETAK</td>
                <td class="meta-value">: {{ now()->translatedFormat('d F Y H:i') }} WIB</td>
            </tr>
            <tr>
                <td class="meta-label">FILTER CETAK</td>
            ...
            <tr>
                ...
                <td class="meta-value">: <strong>{{ $header->status_global ?? 'Draft' }}</strong></td>
                <td class="meta-label">TOTAL item RINCIAN</td>
                <td class="meta-value">: {{ $details->count() }} Item Belanja</td>
            </tr>
        </table>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 12%;">OPERATOR PEMBUAT</th>
                    ...
                    <tr>
                        ...
                        <td class="text-left">
                            ...
                            @if(!empty($detail->description))
                                <br><span style="color: #475569; font-size: 9px;">{{ $detail->description }}</span>
                            @endif
                        </td>
                        <td class="text-center font-bold" style="color: #1e3a8a;">
                            {{ $detail->submission->unit->name ?? '-' }}
                        </td>
                        <td class="text-center" style="color: #334155;">
                            {{ $detail->creator->name ?? 'Operator' }}
                        </td>
                        ...
                        <td class="text-center">
                            @if($detail->is_validated)
                                <span class="badge badge-validated">✓</span>
                ...
                @empty
                    <tr>
                        <td colspan="11" class="text-center" style="padding: 20px; color: #64748b; font-style: italic;">
                            Belum ada rincian belanja usulan yang sesuai dengan filter unit & operator yang dipilih.
                        </td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="5" class="text-right">TOTAL AKUMULASI USULAN:</td>
                ...
                <tr>
                    <td>
                        <div class="sig-title">
                            ...
                            <strong>Direktur / Administrator RSUD Kardinah</strong>
                        </div>
                    ...
                    <td>
                        <div class="sig-title">
                            ...
                            <strong>Tim Anggaran & Penyusun RBA</strong>
                        </div>
                        <div class="sig-name">( ___________________________ )</div>
                        <div class="sig-role">Tim Penyusun RBA Hospital</div>
                    </td>
                </tr>
            </table>
        </div>
