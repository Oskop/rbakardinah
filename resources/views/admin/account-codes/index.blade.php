            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5">
                <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h3 class="text-xs font-extrabold uppercase tracking-wider text-slate-800">Filter Kolom Nomor Rekening</h3>
                            <p class="text-[11px] text-slate-400">Saring rekening belanja berdasarkan kriteria spesifik di bawah ini</p>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <!-- Filter 1: Status Nomor Rekening -->
                    <div>
                        <label for="filter-status" class="block text-xs font-bold text-slate-700 mb-1.5">Status Nomor Rekening</label>
                    ...
                    <!-- Filter 2: Kelompok Belanja -->
                    <div>
                        <label for="filter-kelompok" class="block text-xs font-bold text-slate-700 mb-1.5">Kelompok Belanja Induk</label>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-200/80">
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-auto">
                        <table id="account-codes-table" class="min-w-full divide-y divide-gray-200 text-left text-sm">
                            <thead class="bg-slate-50 border-b border-slate-200">
                                <tr>
                                    <th class="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase tracking-wider">
                                        Kode</th>
                                    <th class="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase tracking-wider">
                                        Nama Rekening Belanja</th>
                                    <th class="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase tracking-wider">
                                        Kelompok Belanja</th>
                                    <th class="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase tracking-wider">
                                        Status</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($accountCodes as $code)
                                    <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td data-search="{{ $code->kelompokBelanja->name ?? '-' }}" 
                                            data-filter="{{ $code->kelompokBelanja->name ?? '-' }}"
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                            @if($code->kelompokBelanja)
                                                <span class="px-2 py-0.5 inline-flex items-center text-xs font-medium rounded bg-slate-100 text-slate-700 border border-slate-200">
                                                    {{ $code->kelompokBelanja->name }}
                                                </span>
                                            @else
                                                <span class="text-gray-400 italic">-</span>
                                            @endif
                                        </td>
                                        <td data-search="{{ $code->is_active ? 'Active' : 'Inactive' }}" 
                                            data-filter="{{ $code->is_active ? 'Active' : 'Inactive' }}" 
                                            class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-bold rounded-full {{ $code->is_active ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-gray-100 text-gray-600 border border-gray-200' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                            ...
                                            <form action="{{ route('admin.account-codes.destroy', $code) }}" method="POST"
                                                class="inline-block"
                                                onsubmit="return confirm('Apakah Anda yakin ingin ' + ({{ $code->is_active ? 'true' : 'false' }} ? 'menonaktifkan' : 'mengaktifkan') + ' nomor rekening {{ addslashes($code->name) }}?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="{{ $code->is_active ? 'text-amber-600 hover:text-amber-900 font-semibold' : 'text-green-600 hover:text-green-900 font-semibold' }} transition-colors">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-8 whitespace-nowrap text-sm text-center text-gray-500">
                                            Tidak ada data nomor rekening ditemukan.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    @push('scripts')
        ...
        <script>
            $(document).ready(function() {
                ...
                const table = $('#account-codes-table').DataTable({
                    responsive: true,
                    columnDefs: [
                    ...
                    language: {
                        search: "Cari Bebas:",
                        searchPlaceholder: "Kode atau Nama Rekening...",
                        lengthMenu: "Tampilkan _MENU_ data",
                        info: "Menampilkan _START_ sampai _END_ dari _TOTAL_ nomor rekening",
                        infoEmpty: "Menampilkan 0 sampai 0 dari 0 nomor rekening",
                        infoFiltered: "(disaring dari _MAX_ total rekening)",
                        paginate: {
                            first: "Pertama",
                            last: "Terakhir",
                            next: "Selanjutnya",
                ...
                // Filter 1: Status Nomor Rekening (Column Index 3)
                $('#filter-status').on('change', function() {
                    const val = $(this).val();
                    table.column(3).search(val ? '^\\s*' + escapeRegex(val) + '\\s*$' : '', true, false).draw();
                });

                // Filter 2: Kelompok Belanja (Column Index 2)
