            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-xl text-gray-800 leading-tight flex items-center gap-2">
                        <span>📢</span>
                        <span>{{ __('Buat Pengumuman Baru') }}</span>
                    </h2>
                    <p class="text-xs text-gray-500 mt-0.5">Atur target pengguna, pesan peringatan, serta durasi dan rentang waktu penayangan.</p>
                </div>
            </div>
            <a href="{{ route('admin.announcements.index') }}" 
    ...
    <div class="py-8">
        <div class="w-full px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
            ...
            <form action="{{ route('admin.announcements.store') }}" method="POST" 
                  x-data="announcementForm()" class="space-y-8">
                @csrf
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="4" required x-model="content"
                                  ...
                                  class="w-full text-sm rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-xs transition-colors">{{ old('content') }}</textarea>
                    </div>
                    ...
                    <div class="p-4 rounded-xl border border-dashed border-gray-300 bg-slate-50/70 space-y-2">
                        ...
                        <div class="p-4 rounded-xl border flex items-start gap-3 shadow-xs transition-all"
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="text-sm font-bold truncate" x-text="title || 'Judul Pengumuman Akan Muncul Di Sini'"></h4>
                                <p class="text-xs mt-1 whitespace-pre-line text-opacity-90 leading-relaxed" x-text="content || 'Isi teks pesan pengumuman akan ditampilkan di area ini secara rapi...'"></p>
                            </div>
                ...
                <div class="bg-white p-6 sm:p-8 rounded-2xl border border-gray-200 shadow-sm space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="start_at" id="start_at" required x-model="startAt"
                                   step="1"
                                   value="{{ old('start_at', now()->format('Y-m-d\TH:i:s')) }}"
                    ...
                    <div class="pt-4 border-t border-gray-100 flex items-center justify-between">
                        <div>
                            <div class="text-xs font-bold text-gray-900">Aktifkan Pengumuman Langsung</div>
                            <div class="text-[11px] text-gray-500">Jika dicentang, pengumuman akan langsung aktif sesuai jadwal yang ditentukan.</div>
                        </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-4">
                    ...
                    <button type="submit" 
                        ...
                        <span>Simpan & Jadwalkan Pengumuman</span>
                    </button>
                </div>
            </form>
    ...
    <script>
        function announcementForm() {
            return {
                ...
                startAt: '{{ old('start_at', now()->format('Y-m-d\TH:i:s')) }}',
                endAt: '{{ old('end_at', '') }}',
                noEndTime: {{ old('end_at') ? 'false' : (old('target_type') ? 'true' : 'false') }},
                userSearch: '',
                selectedUserIds: @json(old('target_user_ids', [])),
                ...
                selectAllSupervisors() {
                    ...
                    spvElements.forEach(el => {
                        ...
                        if (uid && !this.selectedUserIds.includes(uid)) {
                ...
                toggleUnitUsers(unitId) {
                    ...
                    const allInUnitSelected = unitUserIds.every(id => this.selectedUserIds.includes(id));
                    if (allInUnitSelected) {
                        this.selectedUserIds = this.selectedUserIds.filter(id => !unitUserIds.includes(id));
                    } else {
