    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Sub-Unit Kerja: ') }} {{ $subUnit->name }}
            </h2>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-slate-200/80">
                <div class="p-6 md:p-8 text-gray-900">
                    <form method="POST" action="{{ route('admin.sub-units.update', $subUnit) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="unit_id" name="unit_id" required
                                ...
                                @foreach($units as $u)
                                    <option value="{{ $u->id }}" {{ old('unit_id', $subUnit->unit_id) == $u->id ? 'selected' : '' }}>
                                        🏢 {{ $u->name }} ({{ $u->code }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="type" name="type" required
                                class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-xl shadow-sm">
                                @foreach($types as $t)
                                    <option value="{{ $t }}" {{ old('type', $subUnit->type) == $t ? 'selected' : '' }}>
                                        {{ $t }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="code" class="block mt-1 w-full font-mono uppercase" type="text" name="code"
                                :value="old('code', $subUnit->code)" placeholder="Contoh: SUB-REN-01, SUB-FAR-01" />
                        ...
                        <div>
                            ...
                            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name"
                                :value="old('name', $subUnit->name)" required />
                        ...
                        <div>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $subUnit->is_active) ? 'checked' : '' }}
